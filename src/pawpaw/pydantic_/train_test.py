...
class TrainSummary(BaseModel):
    ...
    The term "summary" is used because "result" is just too common, and can be confusing
    when mixed with all other variables.
    """
...
class TestSummary(BaseModel):
    """Result of a model testing/evaluation process from MLFlow, containing the run id,
    model URI, and other useful info.
...
class ModelRegisTags(BaseSettings):
    """Tags to attach (to MLFlow model) when registering a new model version. Used after
    training.
    """
...
class ModelBestTags(BaseModel):
    ...
    best model version. Used after testing/evaluation.
    """
