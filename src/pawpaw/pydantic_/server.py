...
class ServerConf(BaseSettings):
    """The endpoint where the model input data will be send (e.g. from Streamlit), and
    to receive back the prediction result.
    """

    model_config = SettingsConfigDict(validate_by_name = True, validate_default = False, extra = 'allow')

    model_endpoint: Annotated[str, Field(validation_alias = 'SERVER_MODEL_ENDPOINT')] = 'http://localhost:8765/predict'


class ServerRequest(BaseModel):
    """Request that must be posted in order to get the prediction result from a served
    model (e.g. from FastAPI).
    """
